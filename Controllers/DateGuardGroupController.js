...
async function sendSMS(phoneNumber, smsCode, username, token, groupToken, path) {
  try {
    const message = await client.messages.create({
      body: `${username} has requested you to be one of her Guardians Please click on the link and enter the following ${process.env.REACT_SMS_CODE_URL}${path ? path : ""}/${token}/${groupToken} Please Enter this code ${smsCode}`,
      from: twillioNumber,
      to: phoneNumber,
    });
...
const inviteMemberToGroup = async (req, res) => {
  try {
    ...
    console.log(`Please click on the link and enter the following ${process.env.REACT_SMS_CODE_URL}/${member._id}/${groupId} Please Enter this code ${smsCode}`) // for development purpose only
  ...
  try {
    ...
    if (isDecoyed) {
      ...
      for (let member of members) {
        console.log(`Your family member/friend ${user.name || ''} is in danger, he/she decoyed an danger alarm to get more info visit below link ${process.env.REACT_URL}/dateguard/emergency-contacts/${existingAlarm._id.toString()}/${member._id.toString()}`)
        promises.push(sendSMSGeneral(member.phoneNumber, `Your family member/friend ${user.name || ''} is in danger, he/she decoyed an danger alarm to get more info visit below link ${process.env.REACT_URL}/dateguard/emergency-contacts/${existingAlarm._id.toString()}/${member._id.toString()}`))
      }
